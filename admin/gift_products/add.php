...
if (isset($_POST['add_pro'])) {
  ...
  $stmt = $connect->prepare("SELECT * FROM products_gift WHERE slug = ?");
  $stmt->execute(array($slug));
  $count = $stmt->rowCount();
  if ($count > 0) {
    $formerror[] = ' اسم الهدية  موجود من قبل من فضلك ادخل اسم اخر  ';
  }
  // main image
  if (empty($formerror)) {
  ...
  */
  ...
  if (empty($formerror)) {
    $stmt = $connect->prepare("INSERT INTO products_gift (name, slug , description,short_desc,purchase_price,
    price, sale_price , av_num,tags,publish)
    VALUES (:zname,:zslug,:zdesc,:zshort_desc,:zpurchase_price,:zprice,:zsale_price,:zav_num,:ztags,:zpublish)");
    $stmt->execute(array(
      "zname" => $name,
      "zslug" => $slug,
      "zdesc" => $description,
    ...
    $stmt = $connect->prepare("SELECT * FROM products_gift ORDER BY id DESC LIMIT 1");
    $stmt->execute();
...
?>
      ...
      header('Location:main?dir=gift_products&page=add');
    }
  } else {
...
<section class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1 class="m-0 text-dark"> اضافة منتج كهدية </h1>
      </div>
      <!-- /.col -->
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-left">
          <li class="breadcrumb-item"><a href="main.php?dir=dashboard&page=dashboard">الرئيسية</a></li>
          <li class="breadcrumb-item active"> اضافة منتج كهدية</li>
        </ol>
      </div>
      <!-- /.col -->
    </div>
        ...
        <div class="col-md-6">
          <div class="card card-primary">
            <div class="card-body">
              ...
              <div class="form-group">
                <label for="inputStatus"> خصائص الهدية </label>
                <select id="" class="form-control custom-select select2" name="plants_options[]" multiple>
                  <option value="نباتات مجهزة في احواض"> نباتات مجهزة في احواض </option>
                  <option value="أقل من 100 ريال">أقل من 100 ريال</option>
                  <option value="هدية نبتة">هدية نبتة</option>
                  <option value="هدية نبتة مع شوكولاتة">هدية نبتة مع شوكولاتة</option>
                  <option value="هدايا وتوزيعات">هدايا وتوزيعات</option>
                </select>
              </div>
              <div class="form-group">
