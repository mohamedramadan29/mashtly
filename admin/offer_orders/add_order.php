...
if (isset($_POST['add_new_outside_order'])) {
    ...
    try {
        ...
        $stmt = $connect->prepare("SELECT * FROM offer_orders ORDER BY id DESC LIMIT 1");
        $stmt->execute();
        ...
        if (empty($formerror)) {
            if ($payment_method === 'الدفع عن الاستلام') {
                ...
                $stmt = $connect->prepare("INSERT INTO offer_orders (order_number, user_id, name, email,phone,
