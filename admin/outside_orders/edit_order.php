...
if (isset($_GET['order_id']) && is_numeric($_GET['order_id'])) {
    ...
    if (isset($_POST['edit_product_order'])) {
        ...
        if ($product_new_qty != $product_old_qty) {
            $stmt = $connect->prepare("UPDATE outside_order_details SET qty = ? WHERE id = ?");
            ...
            $stmt = $connect->prepare("UPDATE outside_orders SET total_price = ? WHERE id = ?");
            $stmt->execute(array($new_total_price, $order_id));
            if ($stmt) {
                header("Location:main.php?dir=outside_orders&page=edit_order&order_id=" . $order_id);
            }
        }
    }
    ...
    if (isset($_POST['delete_product_order'])) {
        ...
        $stmt->execute(array($new_total_price, $order_id));
        if ($stmt) {
            $stmt = $connect->prepare("DELETE FROM outside_order_details WHERE id = ?");
    ...
    if (isset($_POST['add_products'])) {
        ...
        if (empty($formerror)) {
            ...
            $stmt = $connect->prepare("INSERT INTO outside_order_details (order_id,order_number,product_id,qty,product_price,more_details)
                            ...
                            <div class="tab-content" id="custom-tabs-one-tabContent">
                                <div class="tab-pane fade" id="custom-tabs-one-home" role="tabpanel" aria-labelledby="custom-tabs-one-home-tab">
                                    ...
                                    if (isset($_POST['edit_customer_data'])) {
                                        ...
                                        $stmt = $connect->prepare("UPDATE outside_orders SET name = ?, email = ? , phone = ? , area = ? , address = ? WHERE id = ?");
...
<?php
    ...
    header('Location:main.php?dir=outside_orders&page=report');
    exit();
}
