...
if (isset($_GET['order_id']) && is_numeric($_GET['order_id'])) {
    ...
    if (isset($_POST['edit_product_order'])) {
        ...
        if ($product_new_qty != $product_old_qty) {
            $stmt = $connect->prepare("UPDATE order_details SET qty = ? WHERE id = ?");
            ...
            $stmt = $connect->prepare("UPDATE orders SET total_price = ? WHERE id = ?");
    ...
    if (isset($_POST['delete_product_order'])) {
        ...
        $stmt->execute(array($new_total_price, $order_id));
        if ($stmt) {
            $stmt = $connect->prepare("DELETE FROM order_details WHERE id = ?");
    ...
    if (isset($_POST['add_products'])) {
        ...
        if (empty($formerror)) {
            ...
            $stmt = $connect->prepare("INSERT INTO order_details (order_id,order_number,product_id,qty,product_price,more_details)
...
?>
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark"> تعديل الطلب </h1>
                </div>
                <!-- /.col -->
                <div class="col-sm-6">
