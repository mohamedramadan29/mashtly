...
if (addOrderToGoogleSheet($OrderData)) {
    $stmt = $connect->prepare("UPDATE orders SET add_to_sheet = '1' WHERE id = ?");
    $stmt->execute(array($order_id));
    $_SESSION['success_message'] = " تم إضافة الطلب بنجاح  ";
    $redirect_page = 'main?dir=orders&page=report';
    if(isset($_SESSION['from_page']) && $_SESSION['from_page'] == 'pending_orders'){
        $redirect_page = 'main?dir=orders&page=pending';
    }
